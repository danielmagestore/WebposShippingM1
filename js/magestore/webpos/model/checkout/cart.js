...
define(
    ...
    function ($, ko, Items, Totals, Event, DataManager, CartResource, TaxCalculator, poleHelper, Helper, ProductFactory) {
        ...
        var CartModel = {
            ...
            collectShippingTaxRate: function(){
                var self = this;
                var shippingTaxClass = window.webposConfig["tax/classes/shipping_tax_class"];
                var data = {tax_class_id:shippingTaxClass};
                data = self.collectTaxRate(data);
                if(data.tax_rates){
                    Totals.shippingTaxRates(data.tax_rates);
                }
            },
